...
fontlist = ["dandelion_in_the_spring.ttf", "daniel.ttf", "danielbd.ttf", "danielbk.ttf", "where_stars_shine.ttf"];
...
for font in fontlist:
	...
	f = open(font+"_img.idx", "wb");
	f.write(struct.pack(">I",2051)); #0,0,8,3
	f.write(struct.pack(">I", len(charlist)*1*len(rotatelist) ));
	f.write(struct.pack(">I", 28 ));
	f.write(struct.pack(">I", 28 ));

	l = open(font+"_label.idx", "wb");
	l.write(struct.pack(">I",2049)); #0,0,8,1
	l.write(struct.pack(">I", len(charlist)*len(rotatelist) ));
